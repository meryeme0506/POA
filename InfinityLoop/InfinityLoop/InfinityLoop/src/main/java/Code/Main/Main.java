...
public class Main {
    ...
    try{    
        if( cmd.hasOption( "g" ) ) {
            ...
            Grid grid2 = Generator.generateLevel(outputFile,grid1 );
            System.out.println(grid2);
            GUI gui = new GUI(grid2);
            Grid grid3 = Checker.buildGrid(outputFile);
            System.out.println(grid3);
            System.out.println(Checker.check(grid3));
        }
            ...
            // ...
            Grid grid = Checker.buildGrid("../grids/" + inputFile);
            if (GUI.solve(grid) != null) {
                solved = true;
            }
            assert grid != null;
            Generator.gridToFile("../grids/" + outputFile, grid);
